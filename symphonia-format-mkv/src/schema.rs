...
type MkvSchemaMap = HashMap<u32, MkvElementInfo>;

static MKV_SCHEMA: OnceBox<MkvSchemaMap> = OnceBox::new();

fn init_mkv_schema() -> Box<MkvSchemaMap> {
...
impl EbmlSchema for MkvSchema {
    ...
    fn get_element_info(&self, id: u32) -> Option<&Self::ElementInfo> {
        MKV_SCHEMA.get_or_init(init_mkv_schema).get(&id)
    }
}
