...
static FRAME_READERS: OnceBox<FrameReaderMap> = OnceBox::new();

fn init_frame_readers() -> Box<FrameReaderMap> {
...
fn find_frame_reader(id: [u8; 4]) -> (FrameReader, Option<RawTagParser>) {
    FRAME_READERS
        .get_or_init(init_frame_readers)
        .get(&id)
        .map(|a| a.to_owned())
        .unwrap_or_else(|| null_frame_reader())
}
...
fn find_legacy_frame_reader(id: [u8; 3]) -> (FrameReader, Option<RawTagParser>) {
    match LEGACY_FRAME_MAP.get_or_init(init_legacy_frame_map).get(&id) {
