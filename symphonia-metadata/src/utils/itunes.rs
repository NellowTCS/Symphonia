...
static ITUNES_TAG_MAP: OnceBox<RawTagParserMap> = OnceBox::new();

fn init_itunes_tag_map() -> Box<RawTagParserMap> {
...
pub fn parse_itunes_tag(key: String, value: RawValue, builder: &mut MetadataBuilder) -> Result<()> {
    builder
        .add_mapped_tags(RawTag::new(key, value), ITUNES_TAG_MAP.get_or_init(init_itunes_tag_map));
    Ok(())
}
