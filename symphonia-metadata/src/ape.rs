...
static APE_TAG_MAP: OnceBox<RawTagParserMap> = OnceBox::new();

fn init_ape_tag_map() -> Box<RawTagParserMap> {
...
impl MetadataReader for ApeReader<'_> {
    ...
    fn read_all(&mut self) -> Result<MetadataBuffer> {
        ...
        for _ in 0..header.num_items {
            ...
            if let Some(std_key) = APE_VISUAL_TAG_MAP
                .get_or_init(init_ape_visual_tag_map)
                .get(key_lower.as_str())
                .copied()
            {
            ...
            builder.add_mapped_tags(
                RawTag::new(item.key, value),
                APE_TAG_MAP.get_or_init(init_ape_tag_map),
            );
        }
