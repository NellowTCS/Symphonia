...
static INTENSITY_STEREO_RATIOS_MPEG2: OnceBox<[[(f32, f32); 32]; 2]> = OnceBox::new();

/// Builds the `INTENSITY_STEREO_RATIOS_MPEG2` table on first use.
fn init_intensity_stereo_ratios_mpeg2() -> Box<[[(f32, f32); 32]; 2]> {
    let is_scale: [f64; 2] = [1.0 / (f64::consts::SQRT_2).sqrt(), f64::consts::FRAC_1_SQRT_2];
...
/// table.
static INTENSITY_STEREO_RATIOS_MPEG1: OnceBox<[(f32, f32); 7]> = OnceBox::new();

/// Builds the `INTENSITY_STEREO_RATIOS_MPEG1` table on first use.
fn init_intensity_stereo_ratios_mpeg1() -> Box<[(f32, f32); 7]> {
...
fn process_intensity_long_block(
    ...
    let (is_table, is_inv_pos) = if header.is_mpeg1() {
        (
            &INTENSITY_STEREO_RATIOS_MPEG1.get_or_init(init_intensity_stereo_ratios_mpeg1)[..],
        ...
        (
            &INTENSITY_STEREO_RATIOS_MPEG2.get_or_init(init_intensity_stereo_ratios_mpeg2)
                [usize::from(is_scale)][..],
            INTENSITY_INV_POS_MPEG2,
        )
    };
