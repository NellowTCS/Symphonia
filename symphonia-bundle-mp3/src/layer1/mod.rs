...
static FACTOR: OnceBox<[f32; 16]> = OnceBox::new();

/// Builds the sample dequantization factor table on first use.
fn init_factor() -> Box<[f32; 16]> {
...
impl Layer for Layer1 {
    ...
    fn decode(
        ...
        let factor = FACTOR.get_or_init(init_factor);
